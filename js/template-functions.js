...
$( document ).ready( function(){
    ...
    var coordinates = JSON.parse(mapContainer.getAttribute('data-coordinates'));
    var icon = JSON.parse(mapContainer.getAttribute('data-icon'));
    var info = JSON.parse(mapContainer.getAttribute('data-info'));
    var zoomLevel = parseInt(mapContainer.getAttribute('data-zoom-level'));
    var style = mapContainer.getAttribute('data-style');

    var map = new BMap.Map('baidu-map');
    var point = new BMap.Point(coordinates[0][0], coordinates[0][1]);
    map.centerAndZoom(point, zoomLevel);
    if (style === 'greyscale') {
      map.setMapStyle({ style: 'grayscale' });
    }

    var marker = new BMap.Marker(point, { icon: new BMap.Icon(icon[0], new BMap.Size(32, 32)) });
    map.addOverlay(marker);
    var infoWindow = new BMap.InfoWindow(info[0]);
					...
					$.ajax({
						...
						try {
							if( data.response === true ){
								...
								if ( data.json && data.json.redirect ) {
